...
export default function Agent({ isActive }) {
  const [isTime, setIsTime] = useState(false);
  const [selectedTime, setSelectedTime] = useState("All time");
  const [menu, setMenu] = useState(null);
  const [isDelete, setIsDelete] = useState(false);
  const [viewLoading, setViewLoading] = useState(false);
  const { adminConversations } = useContext(UserDataContext);
  ...
  function handleClick(id) {
    ...
    console.log("Hello");
    console.log("Admin Conversations in Agent component:", adminConversations);
  }
  ...
  const { userData, liveAgentsAdmin, setLiveAgentsAdmin } =
  ...
  function handleView(agent) {
    setViewLoading(true);

    const agentConversations = adminConversations?.filter(
      (conv) => conv.agent_id === agent.id,
    );

    navigate("/messages", { state: { agent, agentConversations } });

    setTimeout(() => setViewLoading(false), 1500);
  }
  return (
    <div className={styles.agent}>
      {mobileView && <span>Live Agents</span>}
      {viewLoading && <Loader />}
        ...
        <div className={settingStyle.div2}>
          ...
          {mobileView ? (
            <MobileAgent
              ...
              liveAgentsAdmin={liveAgentsAdmin}
              setLiveAgentsAdmin={setLiveAgentsAdmin}
              userData={userData}
            />
          ) : (
            <div className={settingStyle.div3}>
              ...
              {liveAgentsAdmin?.agents?.length === 0 ? (
                ...
                liveAgentsAdmin?.agents?.map((agent, id) => (
                  <div className={settingStyle.role2} key={id}>
                    ...
                    {agent.is_busy === true && agent.is_online === false && (
                      <span
                        style={{
                          color: "red",
                          fontSize: "12px",
